...
import static chessgamepiece.ChessPiece.WHITE_COLOR;
import static chessgamepiece.ChessPiece.BLACK_COLOR;
import static chessgamepiece.ChessPiece.PAWN_FIGURE;
...
public class ChessBoard {
    ...
    public boolean moveToPosition(Cell start, Cell end) {
        ...
        if (isNull(figure)) {
            throw new ChessGameException("На клетке " + start + " нет фигуры");
        }
        if (!nowPlayer.equals(figure.getColor())) {
            throw new ChessGameException("Нельзя двигать фигуру противника");
        }
        if (!canMoveToPosition(figure, end)) {
            throw new ChessGameException("Нельзя двигать " + figure.getName() + " " + figure.getPos() + " на " + end);
        }
        ...
        if (isUnderAttack(WHITE_COLOR.equals(figure.getColor()) ? kingWhite : kingBlack)) {
            ...
            throw new ChessGameException("Нельзя оставлять короля под шахом");
        }
    ...
    public boolean castling(Cell cell) {
        ...
        if (isNull(rook)
                ...
                || !king.isFirstPos()
                || !rook.isFirstPos()) {
            throw new ChessGameException("Рокировка с ладьей " + cell + " невозможна");
        }
        if (isUnderAttack(king)) {
            throw new ChessGameException("Рокировка под шахом невозможна");
        }
        ...
        if (!canMoveToPosition(rook, destRook) || nonNull(get(destRook))) {
            throw new ChessGameException("Рокировка через фигуры невозможна");
        }
        ...
        if (isUnderAttack(midKing, enemyColor) || isUnderAttack(destKing, enemyColor)) {
            throw new ChessGameException("Рокировка короля через клетки под атакой невозможна");
        }
