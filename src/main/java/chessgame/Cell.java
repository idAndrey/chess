...
public class Cell {
    ...
    public Cell(int x, int y) {
        if (!isCell(x, y)) {
            throw new ChessGameException("Некорректные координаты клетки " + x + " " + y);
        }
        this.x = x;
        this.y = y;
    }
    ...
    public static Cell of(String cell) throws ChessGameException {
        if (!isCell(cell)) {
            throw new ChessGameException("Некорректная клетка " + cell);
        }
